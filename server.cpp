...
void Server::onMessage(const std::shared_ptr<vsomeip::message>& request) {
      ...
      // Providing the path of textfile.txt
      std::ifstream file("/home/ajith/Downloads/images.jpg", std::ios::binary);
      if (file.is_open()) {
          const size_t fragment_size = 1300; // Setting the fragment size
          ...
          for (size_t i = 0; i < num_fragments; ++i) {
              ...
              custom_packet.filename = "sample.mp4"; // Set your filename logic
          ...
          std::cout << "SERVICE: Sent the fragments of the text file to the client." << std::endl;

          file.close();
      } else {
          std::cerr << "SERVICE: Failed to open the text file." << std::endl;
      }
  }
int main() {
    Server server;
